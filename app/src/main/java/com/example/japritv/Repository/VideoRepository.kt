...
class VideoRepository(private val videoDao: VideoDao) {
    ...
    suspend fun saveVideoData(response: ResponseVideo) {
        val videoDataList = response.data.map { videoData ->
            VideoData(
                ...
                groupid = videoData.idgroup ?: "",
                idPoster = videoData.idposter?:"",
            )
        }
    ...
    suspend fun getVideoByGroupId(id: String): VideoData? {
        return videoDao.getVideoDataById(id)  // ✅ Ambil data dari Room
    }

    suspend fun getPosterById(groupId: String): String? {
        return videoDao.getPosterByGroupId(groupId)
    }
